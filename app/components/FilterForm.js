    ...
    render() {
        return(
            <form className='filter-form'>
                <label htmlFor="sort-by-name">Сортировать по имени</label>
                <input type="checkbox" name="sort" id="sort-by-name" ref={(input) => this._sortByName = input}/>
                <label htmlFor="sort-by-birthday">Сортировать по дате рождения</label>
                ...
                <button type="button" onClick={this._filterHandler.bind(this)}>Применить</button>
            </form>
        );
    }
}